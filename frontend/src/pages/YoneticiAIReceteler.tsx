// src/pages/YoneticiAIReceteler.tsx
...
const temizleHataOzeti = (ozet: string): string => {
  ...
  if (ozet.includes('click') || ozet.includes('Click')) return 'Element tıklanamadı';
  if (ozet.includes('stale')) return 'Element artık sayfada yok';
  return ozet.substring(0, 60) + '...';
};

const YoneticiAIReceteler: React.FC = () => {
  ...
  const enCokHataProje = projeler.length > 0
    ? projeler.map(p => ({
        ad: p.ad,
        sayi: receteler.filter(r => r.proje_id === p.id).length
      })).sort((a, b) => b.sayi - a.sayi)[0]
    : null;
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#F4F3F0' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        <Topbar title="AI Reçeteler" subtitle="Projelerdeki AI hata analizleri" />
        <div style={{ padding: '24px' }}>
          ...
          <div style={{
            ...
            {[
              { label: 'Toplam Reçete', value: receteler.length, bg: '#FCEBEB', color: '#A32D2D' },
              { label: 'Etkilenen Proje', value: new Set(receteler.map(r => r.proje_id)).size, bg: '#EEEDFE', color: '#3C3489' },
              { label: 'En Çok Hata', value: enCokHataProje ? enCokHataProje.ad.split(' ')[0] : '-', bg: '#FAEEDA', color: '#854F0B' },
            ].map((k) => (
              <div key={k.label} style={{
                ...
                <div style={{ fontSize: '28px', fontWeight: 700, margin: '6px 0', color: k.color, fontFamily: 'Arial, sans-serif' }}>{k.value}</div>
              </div>
            ))}
          </div>
                  ...
                  {filtreliReceteler.map((r, i) => (
                    <tr key={i} style={{ borderTop: '1px solid #F4F3F0' }}>
                      ...
                      <td style={{ padding: '12px 18px' }}>
                        <span style={{
                          padding: '3px 10px', borderRadius: '20px', fontSize: '11px',
                          background: '#FCEBEB', color: '#A32D2D', fontFamily: 'Arial, sans-serif'
                        }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ marginBottom: '12px', padding: '14px', background: '#FCEBEB', borderRadius: '10px' }}>
              <div style={{ fontSize: '11px', fontWeight: 600, color: '#A32D2D', marginBottom: '6px', fontFamily: 'Arial, sans-serif' }}>
                Hata Özeti
              </div>
              <div style={{ fontSize: '12px', color: '#791F1F', fontFamily: 'Arial, sans-serif' }}>
                {temizleHataOzeti(secilenRecete.hata_ozeti)}
              </div>
            </div>

            <div style={{ marginBottom: '12px', padding: '14px', background: '#FAFAF8', borderRadius: '10px', border: '1px solid #EDEBE5' }}>
              <div style={{ fontSize: '11px', fontWeight: 600, color: '#333', marginBottom: '6px', fontFamily: 'Arial, sans-serif' }}>
                AI Analizi
              </div>
              <div style={{ fontSize: '12px', color: '#555', lineHeight: 1.8, whiteSpace: 'pre-wrap', fontFamily: 'Arial, sans-serif' }}>
                {secilenRecete.ai_analiz}
              </div>
            </div>

            {secilenRecete.cozum_adimlari?.length > 0 && (
              <div style={{ padding: '14px', background: '#E1F5EE', borderRadius: '10px' }}>
                <div style={{ fontSize: '11px', fontWeight: 600, color: '#0F6E56', marginBottom: '8px', fontFamily: 'Arial, sans-serif' }}>
                  Çözüm Adımları
                </div>
                {secilenRecete.cozum_adimlari.map((adim: string, i: number) => (
                  <div key={i} style={{
                    fontSize: '12px', color: '#085041', padding: '4px 0',
                    fontFamily: 'Arial, sans-serif', display: 'flex', gap: '8px'
                  }}>
...
export default YoneticiAIReceteler;